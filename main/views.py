...
def itemspage(request):
    ...
    if request.method == 'POST':
        ...
        if purchased_item:
            ...
            messages.success(
                request, f'Congratulations, you bought {purchased_item_object.name} for ${purchased_item_object.price}')
...
def registerpage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(
                request, f'You have created account successfully, You are now logged in as {username}')
...
def myitems(request):
    if request.method == 'GET':
        ...
        return render(request, template_name='main/myitems.html', context={'items': items, 'username': username})
    if request.method == 'POST':
        purchased_item = request.POST.get('purchased-item')
        if purchased_item:
            purchased_item_object = Item.objects.get(name=purchased_item)
            purchased_item_object.owner = None
            purchased_item_object.save()
            messages.success(
                request, f'Removed item {purchased_item_object.name}')
